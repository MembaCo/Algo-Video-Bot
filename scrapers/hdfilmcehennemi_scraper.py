...
class HdfilmcehennemiScraper(BaseScraper):
    ...
    def _scrape_movie_from_html(self, soup):
        ...
        try:
            ...
            logger.error(
                "Yedek HTML kazıma yöntemi sırasında hata oluştu.", exc_info=True
            )
            return None
    ...
    def scrape_movie_metadata(self, url):
        try:
            ...
            return self._scrape_movie_from_html(soup)
    ...
    def scrape_movie_links_from_list_page(self, list_url):
        ...
        try:
            response = requests.get(list_url, headers=self.headers, timeout=20)
            response.raise_for_status()
            soup = BeautifulSoup(response.text, "html.parser")
            movie_links = []
    ...
    def scrape_series_data(self, series_url):
        try:
            ...
            response = requests.get(series_url, headers=self.headers, timeout=20)
            response.raise_for_status()
            soup = BeautifulSoup(response.text, "html.parser")
            series_info = {
